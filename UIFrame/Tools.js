...
export default class Tools {
    ...
    shareOrAd(btn,succCb,failCb,type){
	 ...
	 * 自动创建banner
	 */
	createBanner(errorCb,succCb,offectBottom){
    ...
    playBtnShow(btn,callback,durationScale){
        if(G_PlatHelper.isVIVOPlatform()||G_PlatHelper.isOPPOPlatform()||G_PlatHelper.isWINPlatform()){
     ...
     * @param {*} delay 
     */
    bounceScale(target,t1,t2,delay){
        Laya.timer.once(delay,null,function(){
            Laya.Tween.to(target,{scaleX:1.2,scaleY:1.2},t1,Laya.Ease.bounceOut);
            Laya.timer.once(502,this,function(){
                if(!target||target.destroyed){
                    return;
                }
                Laya.Tween.to(target,{scaleX:1,scaleY:1},t2,Laya.Ease.linearIn);
            }.bind(this));
        })
   ...
   createMatrix(boxs,itemName,par,countData,boxSpace,callback){
    ...
    setBoxPos(boxs,boxSpace,countData){
        ...
        if(countData.row%2==0){
            startX-=(countData.row/2-0.5)*boxSpace;
        }else{
            startX-=(countData.row-1)/2*boxSpace;
        }
        ...
        for(var i=0;i<countData.line;i++){
            ...
            for(var j=0;j<countData.row;j++){
                ...
                boxs[index].centerX=startX+boxSpace*j;
                boxs[index].centerY=startY+boxSpace*i;
            }
        }
    }
    ...
    canUseItem(count,type){
        ...
        if(type==1){//钻石
            return false;
            // if(count>G_PlayerInfo.getDiamCount()){
    ...
    createEf(name,pos,par,recycleTime,isAutoRecycle=false){
    ...
    setBtnScaleTween(btn){
        if(this.btnTweens.hasKey(btn)){
            ...
            if(this.stopTween.length>0){
                ...
                let tVals=[];
                tVals.push({time:500,prop:{scaleX:1.2,scaleY:1.2},ease:Laya.Ease.linearNone})
